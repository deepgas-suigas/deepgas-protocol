...
module gas_futures::gfs_governance {
    ...
    const MIN_PROPOSAL_THRESHOLD: u64 = 1000000; // 1M GFS tokens to create proposal
    ...
    // Mint initial tokens (only admin)
    ...
    // Mint tokens for presale (only admin)
    public fun mint_tokens_for_presale(
        ...
        assert!(governance.admin == tx_context::sender(ctx), E_UNAUTHORIZED);
        coin::mint(&mut governance.treasury_cap, amount, ctx)
    }
